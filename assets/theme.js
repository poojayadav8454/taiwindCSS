...
var DialogCloseButton = class extends HTMLButtonElement {
  constructor() {
    ...
    this.addEventListener("click", () => {
      ...
      document.querySelectorAll(".quick-buy-modal").forEach((modals) => {
        modals.removeAttribute("open");
      });
      this.dispatchEvent(
...
document.addEventListener("facet:update", async (event) => {
  ...
  try {
    ...
    // ***************************************************************
    // ✅ Find and execute script tags only inside .product-list

    const productList = tempContent.querySelector(".product-list");
    if (productList) {
      const scriptTags = productList.querySelectorAll("script");
      ...
      // ✅ Re-run Swiper initialization if needed
      ...
      console.log("No .product-list found in tempContent");
    }
    // ✅ Find and execute script tags only inside .product-list Ends
    // ***************************************************
  ...
  this.lastChild.replaceWith(
    document
      ...
      .createContextualFragment(
        formatMoney(variant["price"], currencyFormat).replace(/\.00$/, "")
      )
  );
  this.classList.toggle(
    "text-on-sale",
...
onVariantChanged_fn6 = function (event) {
  ...
  this.lastChild.replaceWith(
    document
      .createRange()
      .createContextualFragment(
        formatMoney(variant["compare_at_price"], currencyFormat).replace(
          /\.00$/,
          ""
        )
      )
...
loadRecommendations_fn = async function () {
  ...
  if (productRecommendationsElement.childElementCount > 0) {
    ...
    // ************************************
    const productList = tempDiv.querySelector(".product-list");

    if (productList) {
      const scriptTags = productList.querySelectorAll("script");

      scriptTags.forEach((roldScript) => {
        const recommentnewScript = document.createElement("script");
        recommentnewScript.type = roldScript.type;

        if (roldScript.src) {
          // ✅ Load external script dynamically
          recommentnewScript.src = roldScript.src;
          recommentnewScript.async = oldScript.async;
        } else {
          // ✅ Execute inline script
          recommentnewScript.textContent = roldScript.textContent;
        }

        // ✅ Append to document body to execute
        document.body.appendChild(recommentnewScript);
      });
...
loadProducts_fn = async function () {
  ...
  if (recentlyViewedProductsElement.childElementCount > 0) {
    ...
    if (productList) {
      ...
      scriptTags.forEach((recentoldScript) => {
        const recentcommentnewScript = document.createElement("script");
        recentcommentnewScript.type = recentoldScript.type;

        if (recentoldScript.src) {
          recentcommentnewScript.src = recentoldScript.src;
          recentcommentnewScript.async = recentoldScript.async;
        } else {
          recentcommentnewScript.textContent = recentoldScript.textContent;
        }

        document.body.appendChild(recentcommentnewScript);
      });
...
document.addEventListener("DOMContentLoaded", function () {
  let buyNowButton = document.querySelector(".buy__now_it_btn");
  if (buyNowButton) {
